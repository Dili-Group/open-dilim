...
8. **Sub-agent cầm trọn lượt.** Orchestrator chọn xong thì sub LÀ người trả lời (prompt + tool của sub thay của root), không loop lồng loop. Không chắc → root tự trả lời.
...
10. **Channel route, không cấp quyền.** `channel` (path webhook) chỉ chọn root agent; quyền luôn theo `identity` backend resolve từ `senderId`. Whitelist `AgentType` + channel lạ rơi về default (không đoán) + agent tự gate tool.
...
14. **Cron = ingress theo thời gian.** Scheduler tự sinh Envelope `source=cron` → tái dùng nguyên pipeline (không path mới). Fire-once (leader-lock + idempotent `msgId`); không bypass quyền (channel whitelist + `identity` auth); output có `target` rõ. Approval-timeout sweep = 1 cron job.
